...
class Formbuilder extends MY_Controller {
    ...
    public function testform($form_id) {
        ...
        //Fetching the id of the fiedls related to the form
        $this->db->select('field_id');
        $this->db->where('form_id', $form_id);
        $field_list = $this->db->get('form_field_rel')->result();

        
        //Setting the fetched in a string
        $id_array = array();
        ...
        //Fetching the field details 
        
        $this->db->select('form_field.*');
        $this->db->select('form_field.* , form_field_option.text,form_field_option.value');
        $this->db->from('form_field');
        $this->db->join('form_field_option', 'form_field.id=form_field_option.form_field_id', 'left');
        $this->db->where("form_field.id IN ({$id_string})");
